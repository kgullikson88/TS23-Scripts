...
def GetModel(data, model, vel=0.0, vsini=15 * Units.cm / Units.km):
    ...
    for i in range(len(data2)):
        ...
        if len(line_indices) > 0:
            ...
            scale = numpy.median(numpy.log(order.y[line_indices] / order.cont[line_indices]) / numpy.log(
    ...
    for modelfile in filenames:
        ...
        for model, data in zip(orders, orders_original):
            if i != 15 and i != 16 and i != 44:
                ...
                chisq += numpy.sum((data.y - model.y * data.cont / model.cont) ** 2 / data.err ** 2) / float(
                    data.size())
                i += 1
